...
class Measurements:
    ...
    max_heel_height: Optional[float] = None
    ...
    def to_dict(self) -> Dict:
        ...
        data = {
            ...
            'shoe_size_unit': self.shoe_size_unit,
            'max_heel_height': self.max_heel_height,
    ...
    def from_dict(cls, data: Dict) -> 'Measurements':
        ...
        return cls(
            ...
            shoe_size_unit=data.get('shoe_size_unit'),
            max_heel_height=data.get('max_heel_height'),
            shoe_width=data.get('shoe_width')
        )
...
class StylePreferences:
    ...
    preferred_heel_height: Optional[List[str]] = None
    shoe_styles: Optional[List[str]] = None
...
class UserProfile:
    ...
    def __init__(self):
        ...
        # Use absolute path to shared user_data directory
        self.base_dir = Path("/Users/pup/dressup/user_data")
        self._ensure_directories()
        self._load_all_profiles()
    
    def _ensure_directories(self) -> None:
        """Ensure necessary directories exist."""
        # Don't create base_dir since it should already exist
    ...
    def is_new_user(self, name: str) -> bool:
        ...
        profile_path = self.base_dir / "profiles" / f"{name}.json"
        profile_path_lower = self.base_dir / "profiles" / f"{name.lower()}.json"
        
        # Also check if profile is already in memory
        if name in self.profiles:
            return False
            
        return not (profile_path.exists() or profile_path_lower.exists())
        ...
        try:
            profile = {
                ...
                'style_preferences': {
                    ...
                    'preferred_heel_height': style_preferences.preferred_heel_height,
                    'shoe_styles': style_preferences.shoe_styles
                },
            ...
            # Save profile to file with original case
            profile_path = self.base_dir / "profiles" / f"{name}.json"
    ...
    def get_style_guidance(self, name: str) -> Dict:
        ...
        return {
            ...
            'preferred_heel_height': style_prefs['preferred_heel_height'],
            'shoe_styles': style_prefs['shoe_styles']
        }
    ...
    def save_profile(self, name: str) -> bool:
        ...
        try:
            ...
            if 'style_preferences' in profile:
                for key in ['secondary_styles', 'favorite_colors', 'favorite_materials', 'preferred_silhouettes', 'preferred_heel_height', 'shoe_styles']:
    ...
    def load_profile(self, name: str) -> Optional[Dict]:
        ...
        try:
            # First check if profile is already in memory
            if name in self.profiles:
                return self.profiles[name]
            
            # Try to load from disk with original case first, then lowercase
            profile_path = self.base_dir / "profiles" / f"{name}.json"
            profile_path_lower = self.base_dir / "profiles" / f"{name.lower()}.json"
            ...
            if profile_path.exists():
                with open(profile_path, 'r') as f:
                    profile = json.load(f)
                    self.profiles[name] = profile
                    logger.info(f"Loaded profile for user: {name}")
                    return profile
            
            # Try lowercase if original case doesn't exist
            if profile_path_lower.exists():
                with open(profile_path_lower, 'r') as f:
                    profile = json.load(f)
            ...
            logger.info(f"Profile not found for user: {name}")
            return None
        except Exception as e:
            logger.error(f"Error loading profile for {name}: {str(e)}")
            return None
